...
| `src/doosan-robot2/` | Doosan ROS 2 외부 의존성 패키지 |
...
`Eye-in-Hand-Camera` 저장소를 ROS 2 워크스페이스 루트로 사용하며, 실제 ROS 2 패키지는 저장소 내부의 `src/` 폴더에 위치합니다.

Doosan ROS 2 패키지는 같은 `src/` 폴더 안에 별도로 설치합니다.
...
| 경로 | 역할 |
|---|---|
| `Eye-in-Hand-Camera/` | 프로젝트 저장소 루트이자 ROS 2 워크스페이스 루트 |
| `src/cobot2/` | 메인 ROS 2 패키지 |
| `src/cobot2_interfaces/` | ROS 2 Action 인터페이스 패키지 |
| `src/doosan-robot2/` | 외부 Doosan ROS 2 의존성 패키지 |
| `arduino/sg90_controller/` | Arduino SG90 외부 액추에이터 코드 |
...
11. `shoot` 동작이 실행되면 Serial 통신으로 Arduino에 외부 액추에이터 명령을 전달합니다.
...
### 7-5. Arduino 기반 외부 액추에이터 연동

`shoot` 동작은 로봇 동작뿐 아니라 Arduino 기반 외부 액추에이터 제어와 연결됩니다.
...
Arduino는 Serial 명령 `1`을 수신하면 SG90 서보모터를 지정된 각도로 왕복 구동합니다.
...
`Eye-in-Hand-Camera` 저장소 자체를 ROS 2 워크스페이스 루트로 사용합니다.
...
Doosan ROS 2 패키지는 저장소 내부 `src/` 폴더에 별도로 설치합니다.
